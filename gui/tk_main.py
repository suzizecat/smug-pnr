...
class MainWindow(Tk):
	def __init__(self, text : str = "Top", **kwargs):
		...
		self.nodes_grid.build_grid(50,50,10,10)
	...
	def on_redraw(self):
		...
		self.canvas.create_rectangle(0,0,800,600,fill="#AFA")

		node_done = list()
		for node in self.nodes_grid.nodes :
			for n in node.neighbors :
				self.canvas.create_line(node.position_x,node.position_y,n.position_x,n.position_y)
				node_done.append(node)
		for node in self.nodes_grid.nodes :
			self.canvas.create_oval(node.position_x-2,node.position_y-2,node.position_x+2,node.position_y+2,fill="red")
		print("done")
